<template>
    <div class="slide_bar">
      ...
      <div class="slide_chat_info">
        <div class="add_chat">
          <el-button icon="plus" style="width:100%" plain @click="openChat">新建对话</el-button>
        </div>
        <div class="chat_box">
          <div class="chat_search">
            <el-input v-model="recordKeyWod" style="width:180px" placeholder="搜索历史记录" suffix-icon="Search" @keyup.enter="getList()"></el-input>
            <el-icon class="del_btn"><Delete /></el-icon>
          </div>
          <ul class="chat_list">
            <li v-for="(item,index) in titleList" :key="index" :class="{'actived':activedIndex===index}">
              <template v-if="editIndex === index">
                <el-input v-model="item.title" placeholder="请输入标题"></el-input>
              </template>
              <template v-else>
                <p class="ellipsis" @click="handleClick(item,index)">{{item.title}}</p>
      ...
      <!-- 新建对话 -->
      <el-dialog title="新建对话" v-model="chatVisible" width="400" :close-on-click-modal="false" align-center>
          <el-form :model="chatForm" label-width="70px" @submit.prevent>
              <el-form-item label="对话标题">
                <el-input v-model="chatForm.title" type="text" placeholder="请输入对话" @keyup.enter="handleAdd"></el-input>
              </el-form-item>
          </el-form>
...
import { getTitleList,createTopic } from "@/api/index.js";
...
getList();
function getList(){
    getTitleList({userId:chatInfo.userId,title:recordKeyWod.value}).then(res=>{
      if(res.code==200){
